...
import ReklamGorselleri from "./ReklamGorselleri";
...
const BOLUMLER = [
  ...
  /*
   * ┌─ "ÖZELLİKLER" ÇOK GENİŞTİ ⚠️ ─────────────────────────────┐
   * │ Tek sekmede 18 anahtar vardı: hizmetler, yorum ayarları  │
   * │ ve arayüz seçenekleri iç içeydi. Aradığın ayarı bulmak   │
   * │ için hepsini okumak gerekiyordu.                           │
   * │                                                              │
   * │ Sekme içinde üç karta bölündü; adı da ne yaptığını        │
   * │ söylüyor.                                                    │
   * └──────────────────────────────────────────────────────────────┘
   */
  { k: "ozellik",  ad: "Hizmetler",  ikon: "grid",    desc: "Hizmetler, yorumlar, okuma deneyimi" },
  ...
  { k: "reklam",   ad: "Reklam",        ikon: "chart",  desc: "Reklam sayfası ve iletişim" },
] as const;
    ...
    <div className="grid gap-5">
      ...
      {bolum === "genel" && (
        <Card className="p-5">
          <CardHead
            title="Site kimliği"
            desc="Sitenin adı ve sloganı. Başlıkta, sekmede ve paylaşımlarda görünür."
          />
      ...
      {bolum === "reklam" && (
        <Card className="p-5">
          <CardHead
            title="Reklam sayfası"
            desc="Reklam vermek isteyenlere gösterilen tanıtım sayfası."
          />
          {sw("ads_rail_enabled", "Yan reklam şeritleri",
              "Sayfaların iki yanındaki dikey reklam sütunu. Kapatınca "
              + "reklamlar silinmez, yalnızca gösterilmez.")}
          {sw("ads_banner_enabled", "Yatay reklam bantları",
              "Haber ve ana sayfadaki yatay reklam alanları.")}

          <Divider className="my-4" />

          {sw("ads_page_enabled", "Reklam sayfası yayında",
              "Kapalıyken /reklam adresi 404 verir. Metin ve görselleri "
              + "hazırlamadan açmayın.")}
          {txt("ads_page_title", "Başlık",
               "Boş bırakılırsa varsayılan başlık kullanılır.")}
          {txt("ads_page_intro", "Tanıtım metni",
               "Sayfanın en üstündeki kısa açıklama.")}
          {txt("reklam_email", "Reklam e-postası",
               "Boşsa genel iletişim adresi kullanılır. İkisi de boşsa "
               + "iletişim bölümü hiç görünmez.")}

          <Divider className="my-4" />
          <ReklamGorselleri
            degerler={{
              ads_shot_1_key: s.ads_shot_1_key,
              ads_shot_2_key: s.ads_shot_2_key,
              ads_shot_3_key: s.ads_shot_3_key,
            }}
            cdnBase={cdnBase}
            onDegisti={(alan, deger) => {
              /* Diğer görsel alanlarıyla aynı desen */
              setS((p2) => ({ ...p2, [alan]: deger }));
              void save(alan as keyof Settings, deger as never);
            }}
          />
      ...
      {bolum === "ozellik" && (
        /*
         * ⚠ ÜÇ AYRI KART.
         * Önce hepsi "Özellikler" başlığı altında tek listeydi:
         * hizmet anahtarları, yorum ayarları ve arayüz seçenekleri
         * karışıyordu. Aradığın ayarı bulmak zordu.
         *
         * Fragment gerekli — bölüm koşulu tek öğe bekliyor.
         */
        <>
        <Card className="p-5">
          <CardHead
            title="Hizmetler"
            desc="Kapatılan hizmet menüden, widget'lardan ve sayfasından tamamen kalkar."
          />
          ...
          {sw("onthisday_enabled", "Tarihte Bugün",
              "Wikipedia kaynaklı günün tarihî olayları.")}
        </Card>

        <Card className="p-5">
          <CardHead
            title="Yorumlar"
            desc="Okur etkileşimi ve denetim."
          />
          {sw("comments_enabled", "Yorumlar açık")}
          {sw("comments_require_approval", "Yorumlar onaydan geçsin",
              "Açarsan TÜM yorumlar beklemede kalır — filtre devre dışı kalır.")}
          {sw("comment_filter_enabled", "Sakıncalı içerik filtresi",
              "Terör, küfür, cinsellik, çocuk istismarı ve tehdit içeren "
              + "yorumlar incelemeye düşer. Terim sözlüğünü Yorumlar "
              + "ekranından yönetebilirsin.")}
        ...
        <Card className="p-5">
          <CardHead
            ...
            desc="Haber sayfasında ve site genelinde görünen öğeler."
          />
          ...
          {sw("ticker_enabled", "Piyasa şeridi",
              "Piyasalar hizmeti kapalıysa şerit de görünmez.")}
