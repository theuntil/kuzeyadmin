...
const BOS: Politika = {
  ...
  requires_consent: false, is_active: true, mobil_goster: true,
  sort_order: 100,
...
export default function PolitikaPanel() {
  ...
  async function kaydet() {
    ...
    const { error } = await sb.rpc("admin_politika_kaydet", {
      p: {
        ...
        mobil_goster: form.mobil_goster,
  ...
  return (
    <div className="grid gap-4">
      ...
      {/* ---- düzenleme formu ---- */}
      {/*
        ┌─ FORM AÇILIR PENCEREDE ⚠️ ─────────────────────────────────┐
        │ Liste ile form aynı sayfada alt alta duruyordu: yönetici  │
        │ formu açınca liste aşağı kayıyor, hangi politikayı        │
        │ düzenlediğini görmek için yukarı çıkması gerekiyordu.     │
        │                                                              │
        │ ⚠ ARKA PLAN KAPATIYOR.                                      │
        │ Dışarı tıklamak formu kapatıyor; kazara kaybetmemek için  │
        │ yalnızca boş alana tıklandığında.                          │
        └──────────────────────────────────────────────────────────────┘
      */}
      {form && (
        <div
          ...
          onClick={(e) => {
            /* Formun içine tıklamak kapatmamalı */
            if (e.target === e.currentTarget) setForm(null);
          }}
        >
        {/* Formun içine tıklamak kapatmasın */}
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 grid gap-2">
            ...
            {/*
              ┌─ MOBİLDE GÖSTER ⚠️ ────────────────────────────────────────┐
              │ Çerez politikası gibi yalnızca siteye ait metinler       │
              │ uygulamada yer kaplıyordu. Yönetici hangisinin           │
              │ uygulamada görüneceğini seçiyor.                          │
              │                                                              │
              │ ⚠ YAYINDA DEĞİLSE ANLAMSIZ.                                │
              │ Yayından kaldırılmış politika zaten hiçbir yerde          │
              │ görünmüyor; anahtar o durumda kapalı.                     │
              └──────────────────────────────────────────────────────────────┘
            */}
            <label className="flex cursor-pointer items-center gap-2.5 text-[13.5px] font-semibold">
              <input
                type="checkbox" className="h-[18px] w-[18px] accent-fg"
                checked={form.mobil_goster && form.is_active}
                disabled={!form.is_active}
                onChange={(e) => setForm((f) =>
                  f ? { ...f, mobil_goster: e.target.checked } : f)}
              />
