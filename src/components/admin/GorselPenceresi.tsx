...
export default function GorselPenceresi({
  ...
  async function dosyaSec(f: File) {
    ...
    try {
      /*
       * ⚠ YÜKLEMEDEN ÖNCE KÜÇÜLTÜLÜYOR.
       * Telefondan çekilmiş bir fotoğraf 4 MB ve 4000 piksel
       * olabiliyor. Sitede en fazla birkaç yüz piksel
       * gösterilecek bir reklam için o boyutu indirmek sayfayı
       * yavaşlatır ve okurun verisini harcar.
       */
      const kucuk = await gorseliKucult(f);
      const { key } = await r2Yukle(kucuk, klasor, f.name);
      setAnahtar(key);
    } catch (e) {
