...
/** Medya kartının tipi — tam ekran önizleme de bunu kullanıyor */
type HaberMedya = HaberDetayVeri["medya"][number];
...
export default function HaberDetay({
  ...
  /*
   * Tam ekran önizleme. `null` iken kapalı; medya nesnesi
   * geldiğinde pencere açılıyor.
   */
  const [buyut, setBuyut] = useState<HaberMedya | null>(null);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {v.medya.length > 0 && (
        <Card className="p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {v.medya.map((m) => (
              /*
               * ⚠ AÇIKLAMA KALDIRILDI, TIKLAMA EKLENDİ.
               * Kartın altındaki açıklama metni ızgarayı
               * dengesiz yükseklikte bırakıyor ve zaten
               * düzenleme ekranında görünüyordu. Burada
               * medyanın kendisi önemli: üstüne basınca tam
               * ekran açılıyor.
               *
               * Hata mesajı istisna — o bilgi burada kalmalı,
               * yoksa bozuk medya sessizce görünmez olur.
               */
              <button
                key={m.id}
                type="button"
                onClick={() => setBuyut(m)}
                title={m.caption ?? "Büyüt"}
                className="overflow-hidden rounded-[14px] bg-chip text-start transition-transform active:scale-[.98]"
              >
                ...
                {m.last_error && (
                  <span className="block p-2 text-[11.5px] leading-snug text-danger">
                    {m.last_error}
                  </span>
                )}
              </button>
      ...
      {/*
        ...
        ⚠ ORAN KORUNUYOR.
        `object-contain` ile görsel kırpılmadan sığdırılıyor;
        dikey fotoğraflar da tamamen görünüyor. Ekranın %90'ı
        sınır — kenarlarda tıklayıp kapatacak yer kalıyor.
      */}
      {buyut && (
        <div
        ...
        >
          ...
          {buyut.type === "video" ? (
            /*
             * ⚠ VİDEO DOSYASI DOĞRUDAN ANAHTARDAN.
             * `medyaOnizleme` poster (görsel) döndürüyor;
             * oynatmak için ham dosya gerekiyor.
             */
            <video
              src={buyut.storage_key ? `${cdn}/${buyut.storage_key}` : undefined}
              poster={medyaOnizleme(buyut, cdn) ?? undefined}
              controls
              autoPlay
          ...
          {buyut.caption && (
            <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-6 text-center text-[13px] text-white">
              {buyut.caption}
            </span>
          )}
        </div>
