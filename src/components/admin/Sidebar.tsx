...
export default function Sidebar({
  groups, userName, role, logoDark, logoLight, siteUrl,
}: {
  groups: NavGroup[];
  userName: string;
  role: string;
  logoDark: string | null;
  /* Açık temada kullanılacak logo — ikisi de gerekli */
    ...
    <>
      <div className="flex items-center gap-3 px-5 py-5">
        <Link href="/" className="flex min-w-0 flex-1 items-center gap-2.5">
          {/*
            ⚠ İKİ LOGO DA BASILIYOR, CSS SEÇİYOR.
            Tema istemcide biliniyor ama logoyu JavaScript ile
            seçmek ilk çizimde yanlış logonun bir an görünmesine
            yol açıyordu. CSS anında doğru olanı gösteriyor.
          */}
          {/*
            ⚠ AÇIK TEMA LOGOSU YÜKLENMEMİŞ OLABİLİR.
            O durumda koyu logo açık zeminde kayboluyordu.
            `kb-logo-ters` sınıfı onu tersine çevirip görünür
            kılıyor — geçici bir çözüm, doğrusu panelden açık
            tema logosunu yüklemek.
          */}
          {(logoLight ?? logoDark) && (
            ...
            <img
              ...
              alt=""
              data-logo-light
              className={`h-7 w-auto object-contain${logoLight ? "" : " kb-logo-ters"}`}
            />
          )}
          {logoDark ? (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={logoDark} alt="" data-logo-dark className="h-7 w-auto object-contain" />
          ) : logoLight ? (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={logoLight} alt="" data-logo-dark
              className="h-7 w-auto object-contain kb-logo-ters" />
          ) : (
      ...
      <div className="flex items-center gap-2 border-t border-white/10 px-4 py-3.5">
        ...
        <Link
          href={siteUrl}
          title="Siteye git"
          className="flex h-8 w-8 items-center justify-center rounded-full text-deep-muted transition-colors hover:bg-ink/[.06] hover:text-deep-ink"
        >
        ...
        <button
          ...
          title="Çıkış yap"
          className="flex h-8 w-8 items-center justify-center rounded-full text-deep-muted transition-colors hover:bg-ink/[.06] hover:text-brand"
        >
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-line bg-surface px-4 py-3 lg:hidden">
        ...
        {/*
          ⚠ "Yönetim" YAZISI KALDIRILDI.
          Logo zaten hangi panelde olunduğunu söylüyor; yazı
          dar ekranda menü düğmesiyle tema düğmesi arasında
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside
            className="absolute left-0 top-0 flex h-full w-[270px] flex-col border-e border-line bg-sidebar"
      ...
      <aside className="sticky top-0 hidden h-dvh w-[248px] shrink-0 flex-col border-e border-line bg-sidebar lg:flex">
        {body}
      </aside>
      <ConfirmDialog
        open={confirmOut}
