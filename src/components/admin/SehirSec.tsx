...
export default function SehirSec({
  ...
  /*
   * ┌─ "YÜKLENİYOR…" SONSUZA KADAR KALIYORDU ⚠️⚠️ ──────────────┐
   * │ İki ayrı hata üst üste bindi.                              │
   * │                                                              │
   * │ 1) Liste yalnızca pencere açılınca çekiliyordu. Ama         │
   * │    düğmedeki şehir adı listeden bulunuyor — liste boşken   │
   * │    ad yok ve "Yükleniyor…" yazıyordu.                       │
   * │                                                              │
   * │ 2) İlk düzeltmemde `yukleniyor` hem KORUMAYA hem            │
   * │    BAĞIMLILIK listesine kondu. Sonuç kilitlenme:            │
   * │      • etki çalışır, `yukleniyor` true olur                 │
   * │      • durum değişince etki YENİDEN çalışır                 │
   * │      • temizlik `iptal = true` yapar                        │
   * │      • gelen veri "iptal edildi" sayılıp ATILIR             │
   * │    Liste hiç dolmuyordu.                                     │
   * │                                                              │
   * │ Artık uçuş kontrolü `ref` ile: durum değişimi etkiyi        │
   * │ tetiklemiyor, istek iptal edilmiyor.                         │
   * └──────────────────────────────────────────────────────────────┘
   */
  const cekiliyor = useRef(false);

  useEffect(() => {
    const gerekli = acik || Boolean(deger);
    if (!gerekli || liste.length > 0 || cekiliyor.current) return;

    cekiliyor.current = true;
    setYukleniyor(true);

    void (async () => {
      try {
        const { data, error } = await sb.rpc("admin_sehir_liste");
        if (error) {
          /*
           * ⚠ HATA SESSİZ GEÇİLMİYOR.
           * Önce hata yutuluyordu ve ekran sonsuza kadar
           * "Yükleniyor…" kalıyordu. Artık en azından
           * "Şehir seç" görünüyor ve sebebi günlüğe düşüyor.
           */
          console.error("[SEHIR] liste alınamadı:", error.message);
          setListe([]);
        } else {
        ...
        setYukleniyor(false);
        cekiliyor.current = false;
      }
    })();
  }, [acik, deger, liste.length, sb]);
    ...
    <>
      ...
      >
        ...
        <span className={secili ? "" : "text-muted2"}>
          {/*
            ⚠ YALNIZCA GERÇEKTEN YÜKLENİRKEN.
            Önce `deger` varsa koşulsuz "Yükleniyor…" yazıyordu;
            istek başarısız olsa bile metin orada kalıyordu.
          */}
          {secili
            ? `${secili.name}${secili.plate_code ? ` · ${secili.plate_code}` : ""}`
            : deger && yukleniyor ? "Yükleniyor…"
            : deger ? "Şehir seçili — açıp doğrulayın"
            : "Şehir seç"}
        </span>
        {deger && (
          <span
