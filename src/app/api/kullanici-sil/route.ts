...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!yanit.ok) {
      ...
      /*
       * ┌─ YETİM KAYIT GERİ BİLDİRİLİYOR ⚠️ ─────────────────────────┐
       * │ Auth silinemediğinde içerik zaten silinmiş oluyor.        │
       * │ Yönetici bunu görmeden devam ederse aynı e-postayla       │
       * │ yeniden kayıt olunamıyor ve sebebi anlaşılmıyor.          │
       * │                                                              │
       * │ `temizlenebilir` alanı panele "sonradan toparlanabilir"   │
       * │ bilgisini veriyor.                                           │
       * └──────────────────────────────────────────────────────────────┘
       */
      return NextResponse.json({
