...
import FiltrePencere from "@/components/admin/FiltrePencere";
import Dashboard from "@/components/admin/Dashboard";
import QrPanel from "@/components/admin/QrPanel";
import DegisiklikKuyrugu from "@/components/admin/DegisiklikKuyrugu";
import ReklamPanel from "@/components/admin/ReklamPanel";
import SabitPanel from "@/components/admin/SabitPanel";
...
export default async function AdminPage({
  ...
  if (key === "overview") {
    /*
     * ┌─ GÖSTERGE PANELİ ⚠️ ───────────────────────────────────┐
     * │ Önce yalnızca sayı kutuları vardı; eğilim görünmüyordu.│
     * │ Artık okunma eğrisi, şehir haritası ve en çok okunan  │
     * │ şehirler var. Tarih aralığı üstten değiştiriliyor.     │
     * │                                                          │
     * │ Veriler istemcide çekiliyor: aralık değiştiğinde       │
     * │ sayfanın tamamı yeniden üretilmesin.                    │
     * └──────────────────────────────────────────────────────────┘
     */
    return shell(
      <>
        <Dashboard siteUrl={cfg.siteUrl} cdnBase={cfg.cdnBase} />
      </>,
    );
  }
  ...
  if (key === "bildirimler") {
    return shell(
      <>
        <PageHead
          title="Bildirimler"
          desc="Mobil uygulamaya gönderilen push bildirimleri ve kuralları."
        />
        <BildirimPanel />
      </>,
    );
  ...
  if (key === "vitrin") {
    return shell(
      <>
        <PageHead
          title="Vitrin"
          desc="Ana sayfanın en üstündeki kaydırılabilir alanda hangi haberlerin sabit duracağını seç."
        />
        <SabitPanel cdnBase={cfg.cdnBase} />
      </>,
    );
  }
  ...
  if (key === "reklamlar") {
    return shell(
      <>
        <PageHead
          title="Reklamlar"
          desc="Sitedeki reklam alanlarını buradan yönet. Şerit ve bant iki ayrı sistem."
        />
        <ReklamPanel cdnBase={cfg.cdnBase} />
      </>,
    );
  }
  ...
  if (key === "qr") {
    return shell(
      <>
        <PageHead
          title="QR kodlar"
          desc="Afiş ve baskılar için QR üret. Kodun gittiği adresi sonradan değiştirebilirsin."
        />
        <QrPanel siteUrl={cfg.siteUrl} />
      </>,
    );
  }
  ...
  if (key === "queue") {
    ...
    return shell(
      <>
        ...
        {/*
          İki ayrı kuyruk:
            • yeni haberler → OnayKuyrugu (articles.status)
            • yayındakinde değişiklik → DegisiklikKuyrugu (taslak)
          Birleştirmek, farklı işlemler gerektirdikleri için
          kafa karıştırıcı olurdu.
        */}
        <div className="mb-6">
          <div className="mb-3 text-[13px] font-bold">Yeni haberler</div>
          <OnayKuyrugu cdn={cfg.cdnBase} siteUrl={cfg.siteUrl} canApprove={isAdmin} />
        </div>

        <div>
          <div className="mb-1 text-[13px] font-bold">
            Yayındaki haberlerde değişiklik
          </div>
          <p className="mb-3 text-[12.5px] text-muted2">
            Yazar düzenledi, yayındaki sürüm henüz değişmedi.
          </p>
          <DegisiklikKuyrugu />
        </div>
      </>,
    ...
    return shell(
      <>
        <PageHead title="Yorumlar"
          desc="Yorumlar anında yayımlanır; yalnızca sakıncalı bulunanlar incelemeye düşer." />

        {/*
          Filtre sözlüğü yorumların hemen üstünde: incelemeye
          düşen bir yorumu görünce sebebini aynı ekranda
          düzeltebiliyorsun.
        */}
        {/*
          ⚠ AÇILIR BÖLÜM YERİNE PENCERE.
          `details` açılınca yorum listesini aşağı itiyor ve
          uzun sözlükte sayfa iyice uzuyordu. Pencere içeriği
          örtüyor, kapanınca liste yerinde kalıyor.
        */}
        <FiltrePencere />
